...
RESULT_DELAY_SHOW_WINDOW = 3

DELETE_GAME(g_Game)
g_Game = CREATE_GAME()

--local UI_FRAME_READY	= "MISSION_READY"
	...
	{	
		...
		SUCCESS =
		{
			enter = function (state, game)
				...
				--util:ShowRaceString("Tutorial_GoodJob", DIALOG_GOOD_JOB, UIFORM_YESNO)				
				--sleep2(200)
				util:ShowGameExitForm(1, RESULT_AUTO_CLOSE_DELAY, RESULT_DELAY_SHOW_WINDOW)
		...
		FAIL_RETRY =
		{
			enter = function (state, game)
				...
				util:StopTimeRecord()
                --sleep2(200)
				util:ShowGameExitForm(2, RESULT_AUTO_CLOSE_DELAY, RESULT_DELAY_SHOW_WINDOW)

				--if game.result == FAIL_TIME then				
				--	util:ShowRaceString("Tutorial_Timeover", DIALOG_TUTORIAL_FAIL, UIFORM_YESNO, '$1['..MISSION_TRY_TIME..']')					
				--else 
				--	util:ShowRaceString("Tutorial_Failed", DIALOG_TUTORIAL_FAIL, UIFORM_YESNO)
				--end
