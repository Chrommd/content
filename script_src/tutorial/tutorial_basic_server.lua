			...
			event = function( state, game, event, strValue, intValue1, intValue2 )
				...
				if event==EVENT_SCRIPT then
					if strValue=="client" then
						...
						elseif intValue1==CS_EVENT_MISSION_GO_NEXT then
							
							local nextMissionID = intValue2
							print('[Server] MISSION_GO_NEXT received. nextMissionID='..nextMissionID..'\n')
							if mgr.ResetGame ~= nil then
								mgr:ResetGame(nextMissionID)
							end
						elseif intValue1==CS_EVENT_MISSION_RETRY then
							print('[Server] MISSION_RETRY received\n')
							if mgr.ResetGame ~= nil then
								mgr:ResetGame(0)
							end
						end
