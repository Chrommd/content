float4 fadeColor = float4( 0.03, 0.08, 0.3, 1.00 ); // 가까운 물의 색
float fadeExp = float( 1.0 ); //가까운 물의 색이 미치는 범위
float fadeDen = float( 1.2 ); //가까이 물의 색이 미치는 강도

float waterColorDen = float( 0.6 ); // 물의 디퓨즈&라이트 맵이 물의 색에 미치는 강도
float3 waveDen = float3( 0.42, 0.362, 0.18 ); // 파도의 세기(높이, 클수록 파도는 높아짐)
float3 waveScale = float3( 0.04, 0.073, 0.22 ); // 파도의 크기(넓이, 작을수록 파도는 커짐)
float3 localWaveSpeed = float3( 0.52, 0.96, 2.0); // 각 파도의 속도(클수록 파도는 빨라짐)
float2 waveDir = float2( 1.8, 1.8); // 파도의 방향(전체 속도 포함)

float diffWave = float(0.2); // 디퓨즈 흐르는 속도

float alphaLow = float( 0.2 ); // 가까운 투명도 물의 투명도
float alphaWidthHigh = float( 1.9 ); // 가까운 투명도가 미치는 먼 범위(1.0이 수평선, 0.0이 카메라 바로 아래)
float alphaWidthLow = float( 0.3 ); // 가까운 투명도가 미치는 가까운 범위(1.0이 수평선, 0.0이 카메라 바로 아래)

float sunPower = float( 0.28 ); // 태양맵의 강도
...
//    bump.xz = (((bump.xz-0.5) * waveDen) * waveDenFromA) * wave1.z;
//    bump.xz = bump.xz * wave1.z;
    bump = normalize(bump);
float4 clrBase0 = tex2D(Diff1Samp, uv_diff1.xy + (bump.xy * 0.01)); 
float waveDenFromA = pow(clrBase0.a,0.5);
...
//   return refl;
float4 ltmap = tex2D(LtMapSamp, uv_ltmap.xy + bump.xz*0.1);
