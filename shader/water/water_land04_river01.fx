float4 fadeColor = float4( 0.05, 0.25, 0.5, 1.00 ); // 가까운 물의 색
float fadeExp = float( 0.9 ); //가까운 물의 색이 미치는 범위
float fadeDen = float( 1.8 ); //가까이 물의 색이 미치는 강도

float waterColorDen = float( 0.5 ); // 물의 디퓨즈&라이트 맵이 물의 색에 미치는 강도
float3 waveDen = float3( 0.512, 0.81, 0.9 ); // 파도의 세기(높이, 클수록 파도는 높아짐)
float3 waveScale = float3( 0.03, 0.053, 0.23 ); // 파도의 크기(넓이, 작을수록 파도는 커짐)
float3 localWaveSpeed = float3( 0.52, 0.96, 2.0); // 각 파도의 속도(클수록 파도는 빨라짐)
float2 waveDir = float2( -1.6, -0.8); // 파도의 방향(전체 속도 포함)

float diffWave = float(0.2); // 디퓨즈 흐르는 속도

float alphaLow = float( 0.2 ); // 가까운 투명도 물의 투명도
float alphaWidthHigh = float( 1.3 ); // 가가운 투명도가 미치는 먼 범위(1.0이 수평선, 0.0이 카메라 바로 아래)
float alphaWidthLow = float( -0.3 ); // 가가운 투명도가 미치는 가까운 범위(1.0이 수평선, 0.0이 카메라 바로 아래)

float sunPower = float( 0.9 ); // 태양맵의 강도
