...
float3 waveDen = float3( 0.212, 0.42, 0.22 ); // 파도의 세기(높이, 클수록 파도는 높아짐)
float3 waveScale = float3( 0.02, 0.083, 0.187 ); // 파도의 크기(넓이, 작을수록 파도는 커짐)
float3 localWaveSpeed = float3( 0.02, 4.37, 12.0); // 각 파도의 속도(클수록 파도는 빨라짐)
float2 waveDir = float2( 82.4, 82.4); // 파도의 방향(전체 속도 포함)

float diffWave = float(0.4); // 디퓨즈 흐르는 속도
float shadowDistort = float(0.0); //그림자가 일그러지는 정도

float alphaLow = float( 0.1 ); // 가까운 투명도 물의 투명도
float alphaWidthHigh = float( 1.6 ); // 가가운 투명도가 미치는 먼 범위(1.0이 수평선, 0.0이 카메라 바로 아래)
