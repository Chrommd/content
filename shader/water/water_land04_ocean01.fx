float4 fadeColor = float4( 0.05, 0.2, 0.1, 1.00 ); // 가까운 물의 색
...
float3 waveDen = float3( 0.412, 0.22, 0.12 ); // 파도의 세기(높이, 클수록 파도는 높아짐)
float3 waveScale = float3( 0.002, 0.023, 0.077 ); // 파도의 크기(넓이, 작을수록 파도는 커짐)
float3 localWaveSpeed = float3( 0.2, 0.6, 1.0); // 각 파도의 속도(클수록 파도는 빨라짐)
float2 waveDir = float2( -0.45, -0.45); // 파도의 방향(전체 속도 포함)

float diffWave = float(0.2); // 디퓨즈 흐르는 속도

float alphaLow = float( 0.99 ); // 가까운 투명도 물의 투명도
float alphaWidthHigh = float( 0.6 ); // 가가운 투명도가 미치는 먼 범위(1.0이 수평선, 0.0이 카메라 바로 아래)
float alphaWidthLow = float( 0.0 ); // 가가운 투명도가 미치는 가까운 범위(1.0이 수평선, 0.0이 카메라 바로 아래)

float sunPower = float( 0.58 ); // 태양맵의 강도
